...
def train():
    ...
    data_path = "/home1/garudaia/main_proj/alpaca_trainset_200.json"
    ...
    for n, p in model.named_parameters():
        # if ('wq.weight' or 'wq.bias' or 'wv.weight' or 'wv.bias') in n:
    ...
    for epoch in range(5):
        for i, batch in enumerate(dataloader):
            ...
            # print(f"i = {i} of epoch - {epoch}\n")
            if ((i + 1) % grad_acc_step == 0):
                scaler.step(optimizer)
                scaler.update()
                optimizer.zero_grad()
                print(f"accumulated loss after 8 samples - {acc_loss}\n")
                acc_loss = 0

            print(f"epoch {epoch}  sample - {i}   loss - {loss.item()}")
    
    print(f"Training Done!!! Saving the Model -------\n")
    torch.save(model.state_dict(), 'llama_finetune_proj_Final_200.pth')
    print(f"Model Saved!   Running Test - ")
    ...
    for prompt, result in zip(prompts, results):
        print(prompt)
        print(f"The response according to dataset - /n {prompt_print[i]['output']}")
        print(f">Result generated by trained model - /n {result['generation']}")
        print("\n==================================\n")
        i += 1
